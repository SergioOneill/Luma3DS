...
void detectAndProcessExceptionDumps(void)
{
    ...
    if(dumpHeader->processor == 11 && dumpHeader->type >= 2)
    {
        ...
        for(u32 i = 0; i < 15; i++)
            ...
            {
                posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Estado del error:    %s", faultStatusNames[i]);
                break;
            }
    }
    ...
    if(dumpHeader->additionalDataSize != 0)
    {
        ...
        if(dumpHeader->processor == 11)
            ...
            posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE,
                                    "Arm9 volcado de memoria en el offset %X tamaño %lX", (uintptr_t)additionalData - (uintptr_t)dumpHeader, size);
    }
    posY += SPACING_Y;
    ...
    if(dumpHeader->processor == 11 && dumpHeader->type == 3)
        posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "%-7s%08lX       Access type: %s", "FAR", regs[19], regs[17] & (1u << 11) ? "Escribir" : "Leer");

    posY += SPACING_Y;

    u32 mode = regs[16] & 0xF;
    if(dumpHeader->type == 3 && (mode == 7 || mode == 11))
        posY = drawString(true, 10, posY + SPACING_Y, COLOR_YELLOW, "Volcado de datos incorrecto: Error al volcar codigo") + SPACING_Y;
