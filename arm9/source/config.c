...
static const char *keyNames[] = {
    "A", "B", "Select", "Start", "Right", "Left", "Arriba", "Abajo", "R", "L", "X", "Y",
    "?", "?",
    "ZL", "ZR",
    "?", "?", "?", "?",
    "Touch",
...
static size_t saveLumaIniConfigToStr(char *out)
{
    ...
    if (VERSION_BUILD != 0) {
        sprintf(lumaVerStr, "Luma3DS 11 v%d.%d.%d ESP", (int)VERSION_MAJOR, (int)VERSION_MINOR, (int)VERSION_BUILD);
    } else {
        sprintf(lumaVerStr, "Luma3DS 11 v%d.%d ESP", (int)VERSION_MAJOR, (int)VERSION_MINOR);
    }
...
void configMenu(bool oldPinStatus, u32 oldPinMode)
{
    ...
    drawFormattedString(false, 10, SCREEN_HEIGHT - 2 * SPACING_Y, COLOR_YELLOW, "Arrancado desde %s via %s", isSdMode ? "SD" : "CTRNAND", bootTypes[(u32)bootType]);
